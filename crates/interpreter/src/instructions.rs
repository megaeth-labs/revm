...
pub fn eval<H: Host, S: Spec>(
    ...
    match opcode {
        ...
        opcode::LOG4 => host::log::<4>(interp, host),
        opcode::SELFDESTRUCT => host::selfdestruct::<S>(interp, host),
        opcode::CREATE => host::create::<false, S>(interp, host, create_cycles), //check
        opcode::CREATE2 => host::create::<true, S>(interp, host, create_cycles), //check
        opcode::CALL => host::call::<S>(interp, host, call_cycles),              //check
        opcode::CALLCODE => host::call_code::<S>(interp, host, call_cycles),     //check
        opcode::DELEGATECALL => host::delegate_call::<S>(interp, host, call_cycles), //check
        opcode::STATICCALL => host::static_call::<S>(interp, host, call_cycles), //check
        opcode::CHAINID => host_env::chainid::<S>(interp, host),
        _ => return_not_found(interp, host),
    }
}
