...
impl<DB: Database> Database for State<DB> {
    ...
    fn salt_bucket_capacity(
        ...
        // For salt storage, we don't have it in cache, so we directly get it from database.
        self.database.salt_bucket_capacity(address, index)
    }
}
...
impl<DB: DatabaseRef> DatabaseRef for State<DB> {
    ...
    fn salt_bucket_capacity_ref(
        ...
        // For salt storage, we don't have it in cache, so we directly get it from database.
        self.database.salt_bucket_capacity_ref(address, index)
    }
}
