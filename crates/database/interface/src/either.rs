    ...
    R: Database<Error = L::Error>,
{
    ...
    fn salt_bucket_capacity(
        ...
        match self {
            Self::Left(db) => db.salt_bucket_capacity(_address, _index),
            Self::Right(db) => db.salt_bucket_capacity(_address, _index),
        }
    }
}
    ...
    R: DatabaseRef<Error = L::Error>,
{
    ...
    fn salt_bucket_capacity_ref(
        ...
        match self {
            Self::Left(db) => db.salt_bucket_capacity_ref(address, index),
            Self::Right(db) => db.salt_bucket_capacity_ref(address, index),
        }
    }
}
