...
pub trait DatabaseAsync {
    ...
    /// Returns the salt bucket capacity for the given address and optional storage key.
    fn salt_bucket_capacity_async(
        &self,
        _address: Address,
        _index: Option<StorageKey>,
    ) -> impl Future<Output = Result<(usize, u64), Self::Error>> + Send {
        async { unimplemented!("salt bucket capacity is not implemented for this database") }
    }
}
...
pub trait DatabaseAsyncRef {
    ...
    /// Returns the salt bucket capacity for the given address and optional storage key.
    fn salt_bucket_capacity_async_ref(
        &self,
        _address: Address,
        _index: Option<StorageKey>,
    ) -> impl Future<Output = Result<(usize, u64), Self::Error>> + Send {
        async {
            unimplemented!("salt bucket capacity is not implemented for this database reference")
        }
    }
}
...
impl<T: DatabaseAsync> Database for WrapDatabaseAsync<T> {
    ...
    fn salt_bucket_capacity(
        &self,
        address: Address,
        index: Option<StorageKey>,
    ) -> Result<(usize, u64), Self::Error> {
        self.rt
            .block_on(self.db.salt_bucket_capacity_async(address, index))
    }
}
...
impl<T: DatabaseAsyncRef> DatabaseRef for WrapDatabaseAsync<T> {
    ...
    fn salt_bucket_capacity_ref(
        ...
        self.rt
            .block_on(self.db.salt_bucket_capacity_async_ref(address, index))
    }
}
